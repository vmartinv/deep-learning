...
(X_train, Y_train) = get_images('dataset/train')#, tam = 0.01)
...
#~ thresh = 0.4
#~ def binarize(img):
    #~ return np.greater(img, 1-thresh)
def binarize(img):
    return np.greater(img, threshold_otsu(img))

def skeletonize(img):
    img[0] = medial_axis(img[0])
    return img

# binarize
X_train = [np.logical_not(binarize(i)) for i in X_train]
X_test = [np.logical_not(binarize(i)) for i in X_test]
X_valid = [np.logical_not(binarize(i)) for i in X_valid]

#skeletonize
pool = multiprocessing.Pool(processes=multiprocessing.cpu_count())
X_train = pool.map(skeletonize, X_train)
X_test = pool.map(skeletonize, X_test)
X_valid = pool.map(skeletonize, X_valid)
pool.close()
